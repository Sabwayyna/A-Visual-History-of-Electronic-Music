...
print(f"Fixing duplicate start overlay scripts in {len(files)} song files...")
...
for file in files:
    ...
    # Remove ALL existing start overlay scripts (including duplicates)
    # Match from "START OVERLAY SCRIPT" comment to the end marker
    content = re.sub(
        r'//\s*(?:DEBUG VERSION - )?Start overlay.*?===\s*START OVERLAY SCRIPT.*?COMPLETE\s*===.*?\n',
        '',
        content,
        flags=re.DOTALL
    )

    # Also remove any orphaned IIFE patterns that might be left
    content = re.sub(
        r'\(function\(\)\s*\{.*?let\s+audioLoaded\s*=\s*false;.*?\}\)\(\);',
        '',
        content,
        flags=re.DOTALL
    )

    # Find the last </script> tag and insert the clean version before it
...
print("\n✅ Removed duplicates and installed clean start overlay handler!")
