...
function generateFilename(artist, track, year) {
  const combined = `${artist}-${track}-${year}`
    .toLowerCase()
    .replace(/[^\w\s-]/g, '')
    .replace(/\s+/g, '-')
    .replace(/-+/g, '-');
...
// HTML template
function generateHTML(song, filename) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <div class="bottom-nav">
        <!-- Visualizer Icon -->
        <a href="../visualizer.html" class="nav-link" id="visualizerLink">
        ...
        <!-- Alphabet Chart Icon -->
        <a href="../alphabet-chart.html" class="nav-link" id="alphabetLink">
            <svg viewBox="0 0 42 42" xmlns="http://www.w3.org/2000/svg">
                <g stroke="black" stroke-width="1.5" fill="none">
                    <!-- Grid pattern -->
    ...
    <script>
        // Simple tooltip positioning for the second icon
        ...
        navLinks.forEach((link, index) => {
            link.addEventListener('mouseenter', function() {
                if (tooltips[index]) {
                    ...
                    tooltips[index].style.left = rect.left + 'px';
                }
            });
        });
    </script>
...
songs.forEach((song, index) => {
  const filename = generateFilename(song.artist, song.track, song.year);
  const filepath = path.join(songsDir, filename);
  const html = generateHTML(song, filename);
...
fs.writeFileSync(
  path.join(__dirname, 'song-mapping.json'),
  JSON.stringify(mapping, null, 2)
);

console.log(`\nGenerated ${songs.length} song pages successfully!`);
console.log('Mapping saved to song-mapping.json');
